...
export class FollowersService {
    ...
    async toggleFollow(followerId: string, followingId: string) {
        ...
        const existingFollow = await this.drizzle.query.followers.findFirst({
            where: and(
        ...
        if (existingFollow) {
            await this.drizzle.delete(schema.followers)
                .where(
                    and(
                        eq(schema.followers.follower_id, followerId),
                        eq(schema.followers.following_id, followingId)
        ...
        await this.drizzle.insert(schema.followers).values({
